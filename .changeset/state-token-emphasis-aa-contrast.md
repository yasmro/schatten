...
- **コンポーネント影響**: Callout / Toast / Badge の `subtle`（および Badge `outline`）の色文字、`Text color="error|success|warning|info"` の standalone 色文字が `base` → `emphasis` に変更。塗り・ボーダー・`foreground`（solid の白文字）・`subtle` 背景は不変。
...
- `--color-destructive-emphasis` は define-only（`destructive` の消費先 `Button` は solid のみ）。5 state で 5-token 形を揃えるため宣言・`@theme` 登録。
