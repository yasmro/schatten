...
| `Avatar` | — | (none) | — | Out of pattern. A `@radix-ui/react-avatar` wrapper with `size` (`sm`/`md`/`lg`) only — no `variant`/`appearance`. Flat API (`src` / `alt` / `fallback` as a `string` / `delayMs`) over the three Radix parts; the sanctioned flat-over-multi-part-Radix exception, see [component-architecture §2](component-architecture.md#2-compound-vs-flat). Circle shape fixed in v1. |
| `Card` | — | `filled` · `plain` | — | Out of pattern. A surface container; `variant` toggles the background: `filled` (default — `surface` fill + `--shadow-card`) / `plain` (transparent, no fill or shadow, border only — for nested / low-emphasis grouping). Single axis like `Spinner`; no role and no state tone, so neither Pattern A nor B. No `appearance`. See #429. |
...
| `required` | `boolean` | HTML standard. No `is` prefix. Inside a `<Field required>`, the field's `required` propagates to the control as `aria-required` (announce-only — it does **not** enable native validation; form-submission blocking comes only from a `required` prop set directly on the control). See [field-context-guideline](field-context-guideline.md#required-propagation-via-aria-required-announce-only). | Form components — `Input` / `Textarea` / `Select` / `Checkbox` / `Switch` / `Radio` (`RadioGroup`), standalone or inside a `<Field>`. |
