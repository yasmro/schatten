...
`,d=Object.values(t).map(e=>{const[o="",n=""]=(e.description??"").split(": ");return{id:e.name,label:e.name.replace("season--","").split("-").map(r=>r.charAt(0).toUpperCase()+r.slice(1)).join(" "),term:o,colors:n}});function a(e,o){const n=e.replace(/\/\*[\s\S]*?\*\//g,""),r=n.indexOf(o);if(r===-1)throw new Error(`scoped-theme-css: block "${o}" not found in semantic.css`);const l=n.slice(r,n.indexOf("}",r)).match(/--color-solid[\w-]*:\s*[^;]+;/g);if(!l||l.length===0)throw new Error(`scoped-theme-css: no --color-solid* declarations in "${o}"`);return l.join(`
  `)}function m(e,o){const n=o.seasonalCss.replace(/:root\[/g,`${e}[`),r=[`${e}[data-theme] {
  ${a(o.semanticCss,":root {")}
}`,`${e}.dark[data-theme] {
  ${a(o.semanticCss,".dark {")}
}`].join(`
`);return`${n}
