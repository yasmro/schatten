import{S as l}from"./index-BjhfJjZn.js";const i=`/*
 * Semantic Tokens (lv2)
...
:root {
  ...
  /* Muted resting surface — sits between surface and the page, used by
   * Card variant="subtle". Mode-owned; NOT a hover state (so distinct from
   * --color-surface-hover, whose name encodes interaction). */
  --color-surface-subtle: var(--alabaster-100);
  ...
  /* emphasis: readable colored destructive text on a non-solid surface —
     consumed by the DropdownMenu destructive item (red text on the menu
     surface). Same AA contract as the other states. See state-token-guideline.md. */
  --color-destructive-emphasis: var(--red-700);
  ...
  /*
   * State tokens — each provides { base, hover, foreground, subtle, emphasis }.
   *   base       : main color (saturated)         — fills, borders, icons
   *   hover      : darker/lighter sibling of base — interactive hover
   *   foreground : on-base text/icon color        — readable atop base (solid)
   *   subtle     : faint tinted background        — soft state surfaces
   *   emphasis   : readable colored TEXT          — colored text on subtle/light
   *                                                 surfaces (1 rung darker than
   *                                                 base in light, lighter in dark)
   *
   * Why \`emphasis\` exists (#344 Phase B): \`base\` (-600 light) doubles as fill
   * AND text. As text on a \`-subtle\` (-50) or white surface it lands at
   * ~4.2–4.4:1 — below WCAG 1.4.3 small-text 4.5:1. \`emphasis\` splits the text
   * role off \`base\` so the colored text can darken to -700 (light) / -400
   * (dark) and clear AA without darkening the fill. It shares a primitive value
   * with \`hover\` but is a distinct semantic (text vs. interactive fill) — the
   * same same-value / distinct-name policy as \`destructive\` vs \`error\`.
   * \`foreground\` (white-on-fill solid text) is unchanged: the solid trilemma
   * (white + vivid + 4.5) is unsolvable, so solid keeps its intentional
   * high-emphasis treatment with meaning carried by icon + label (1.4.1).
   */
  ...
  --color-error-emphasis: var(--red-700);
  ...
  --color-success-emphasis: var(--green-700);
  ...
  --color-info-emphasis: var(--blue-700);
...
@media (prefers-color-scheme: dark) {
  :root:not(.light) {
    ...
    --color-surface-subtle: var(--alabaster-800);
    ...
    --color-destructive-emphasis: var(--red-400);
    ...
    /* State tokens — dark mode mappings.
       emphasis goes one rung LIGHTER than base (-400) so colored text clears AA
       on the dark \`-900\` subtle surface (base -500 lands 4.2–4.4). */

    --color-error: var(--red-500);
    --color-error-hover: var(--red-400);
    --color-error-foreground: var(--paper-white-inverted);
    --color-error-subtle: var(--red-900);
    --color-error-emphasis: var(--red-400);
    ...
    --color-success-emphasis: var(--green-400);
...
`,d=Object.values(l).map(e=>{const[r="",n=""]=(e.description??"").split(": ");return{id:e.name,label:e.name.replace("season--","").split("-").map(o=>o.charAt(0).toUpperCase()+o.slice(1)).join(" "),term:r,colors:n}});function t(e,r){const n=e.replace(/\/\*[\s\S]*?\*\//g,""),o=n.indexOf(r);if(o===-1)throw new Error(`scoped-theme-css: block "${r}" not found in semantic.css`);const a=n.slice(o,n.indexOf("}",o)).match(/--color-solid[\w-]*:\s*[^;]+;/g);if(!a||a.length===0)throw new Error(`scoped-theme-css: no --color-solid* declarations in "${r}"`);return a.join(`
  `)}function m(e,r){const n=r.seasonalCss.replace(/:root\[/g,`${e}[`),o=[`${e}[data-theme] {
  ${t(r.semanticCss,":root {")}
}`,`${e}.dark[data-theme] {
  ${t(r.semanticCss,".dark {")}
}`].join(`
`);return`${n}
${o}`}export{d as S,i as a,m as b,h as s};
