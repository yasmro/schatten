...
}`,...m.parameters?.docs?.source},description:{story:'`selected` highlights a row via `data-state="selected"` — a surface tint plus\na left accent bar, so the selected row stays distinct even on a striped or\nhoverable table (where the tint alone would blend into the stripe).',...m.parameters?.docs?.description}}};const E=["Playground","Treatments","Sizes","Hoverable","SelectedRow"];export{i as Hoverable,d as Playground,m as SelectedRow,n as Sizes,o as Treatments,E as __namedExportsOrder,C as default};
