import{j as e,r as m}from"./iframe-Q-7Zj3Ct.js";import{P as u,L as h,S as s,N as t}from"./docs-ui-D0Lzeezu.js";import"./preload-helper-DUSBS5vW.js";const N={title:"Tokens/Motion",parameters:{layout:"fullscreen"}},f=[{token:"fast",value:"100ms",motion:"quick",use:"Hover, exit transitions"},{token:"base",value:"150ms",motion:"base",use:"Default, enter transitions"},{token:"slow",value:"200ms",motion:"expressive",use:"Dialog / drawer enter"}],b=[{name:"quick",alias:"fast",status:"defined-only",usedBy:"— (future: hover feedback — hardcoded today)"},{name:"base",alias:"base",status:"defined-only",usedBy:"— (future: default interactions — hardcoded today)"},{name:"expressive",alias:"slow",status:"defined-only",usedBy:"— (future: dialog / drawer — enter uses --st-duration-slow today)"}],j=({status:n})=>n==="applied"?e.jsx("span",{className:"rounded-full bg-success-subtle px-2 py-0.5 text-xs font-medium text-success-emphasis",children:"applied"}):e.jsx("span",{className:"rounded-full bg-warning-subtle px-2 py-0.5 text-xs font-medium text-warning-emphasis",children:"defined only"}),v=()=>{const[n,l]=m.useState(!1),[r,x]=m.useState(!1);return m.useEffect(()=>{const o=window.matchMedia("(prefers-reduced-motion: reduce)");x(o.matches);const p=g=>x(g.matches);return o.addEventListener("change",p),()=>o.removeEventListener("change",p)},[]),e.jsxs("div",{className:"rounded-xl border border-border p-6",children:[e.jsx("button",{type:"button",onClick:()=>l(o=>!o),className:"rounded-md bg-foreground px-4 py-2 text-sm font-medium text-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",children:n?"Reset knobs":"Run transition"}),r&&e.jsxs(t,{children:[e.jsx("code",{children:"prefers-reduced-motion: reduce"})," is active — the demo runs without a transition, so motion is never forced on you."]}),e.jsx("div",{className:"mt-5 flex flex-col gap-4",children:f.map(o=>e.jsxs("div",{className:"flex items-center gap-4",children:[e.jsxs("span",{className:"w-40 shrink-0 text-xs font-mono text-foreground",children:["--st-duration-",o.token]}),e.jsx("div",{className:"relative h-8 flex-1 overflow-hidden rounded-lg border border-border bg-surface",children:e.jsx("div",{"aria-hidden":"true",className:"absolute top-1 h-6 w-6 rounded bg-theme-500",style:{left:n?"calc(100% - 1.75rem)":"0.25rem",transition:r?"none":`left var(--st-duration-${o.token}) ease`}})}),e.jsx("span",{className:"w-16 shrink-0 text-right text-xs font-mono text-foreground-muted tabular-nums",children:o.value})]},o.token))})]})},a={name:"Duration Scale",render:()=>e.jsxs("div",{className:"max-w-3xl mx-auto px-8 py-12",children:[e.jsx(u,{children:"Motion"}),e.jsxs(h,{children:["Schatten centralizes enter/exit timing in a three-step duration scale, defined in"," ",e.jsx("code",{children:"src/core/tokens/animation.css"})," and registered as part of the public token surface via the public-token registrar (",e.jsx("code",{children:"src/styles/public-tokens.css"}),"). Components reference the scale — or its semantic ",e.jsx("code",{children:"--motion-*"})," aliases (see"," ",e.jsx("strong",{children:"Semantic Tokens"}),") — never a hardcoded millisecond value."]}),e.jsx(s,{children:"Duration scale"}),e.jsxs(t,{children:["The raw ",e.jsx("code",{children:"--st-duration-*"})," steps. ",e.jsx("code",{children:"fast"})," / ",e.jsx("code",{children:"base"})," /"," ",e.jsx("code",{children:"slow"})," are perceptually distinct; reach for a semantic ",e.jsx("code",{children:"--motion-*"})," ","alias in usage. Press ",e.jsx("strong",{children:"Run transition"})," to feel the difference — the faster knob settles before the slower one."]}),e.jsx(v,{}),e.jsx("div",{className:"mt-6 border border-border rounded-xl px-5",children:f.map(n=>e.jsxs("div",{className:"flex items-center gap-4 py-3 border-b border-border last:border-b-0",children:[e.jsxs("p",{className:"w-44 shrink-0 text-sm font-medium text-foreground font-mono",children:["--st-duration-",n.token]}),e.jsx("code",{className:"w-16 shrink-0 text-xs text-foreground-muted tabular-nums",children:n.value}),e.jsxs("code",{className:"w-32 shrink-0 text-xs text-foreground-muted",children:["→ --motion-",n.motion]}),e.jsx("p",{className:"flex-1 text-sm text-foreground-muted",children:n.use})]},n.token))})]})},i={name:"Semantic Tokens",render:()=>e.jsxs("div",{className:"max-w-3xl mx-auto px-8 py-12",children:[e.jsx(u,{children:"Semantic tokens"}),e.jsxs(h,{children:["Components reference these intent-named tokens, not the raw scale. Each is a value-preserving alias over a ",e.jsx("code",{children:"--st-duration-*"})," step — the table reads as"," ",e.jsx("strong",{children:"purpose → which duration step → which component uses it"}),". All three are"," ",e.jsx("em",{children:"defined only"})," today: nothing references ",e.jsx("code",{children:"--motion-*"})," yet."]}),e.jsx("div",{className:"border border-border rounded-xl px-5",children:b.map(n=>e.jsxs("div",{className:"flex items-center gap-4 py-4 border-b border-border last:border-b-0",children:[e.jsxs("div",{className:"flex-1",children:[e.jsxs("p",{className:"text-sm font-medium text-foreground font-mono",children:["--motion-",n.name]}),e.jsxs("p",{className:"text-xs text-foreground-muted font-mono",children:["= --st-duration-",n.alias]}),e.jsx("p",{className:"text-xs text-foreground-muted mt-1",children:n.usedBy})]}),e.jsx("div",{className:"w-28 shrink-0",children:e.jsx(j,{status:n.status})})]},n.name))}),e.jsx(s,{children:"Define-only is intentional"}),e.jsxs(t,{children:["The ",e.jsx("code",{children:"--motion-*"})," aliases are ",e.jsx("strong",{children:"defined but not yet applied"}),": component hover transitions are still hardcoded, and unifying them onto the motion scale is a separate spike. The tokens wait for that consumer rather than being wired in piecemeal. This mirrors ",e.jsx("code",{children:"--radius-control"})," in ",e.jsx("strong",{children:"Tokens/Radius"})," and"," ",e.jsx("code",{children:"--shadow-card"})," in ",e.jsx("strong",{children:"Tokens/Elevation"})," — a single-value semantic token may sit define-only until its natural consumer lands (see css-api.md)."]})]})},d=({surface:n,token:l,note:r})=>e.jsxs("div",{className:"flex items-start gap-4 py-3 border-b border-border last:border-b-0",children:[e.jsx("p",{className:"w-28 shrink-0 text-sm font-medium text-foreground",children:n}),e.jsx("code",{className:"w-44 shrink-0 text-xs text-foreground-muted",children:l}),e.jsx("p",{className:"flex-1 text-sm text-foreground-muted",children:r})]}),c={name:"Usage Guide",render:()=>e.jsxs("div",{className:"max-w-3xl mx-auto px-8 py-12",children:[e.jsx(u,{children:"Usage guide"}),e.jsxs(h,{children:["Pick a duration by what the motion is ",e.jsx("em",{children:"for"}),", not by an absolute millisecond count. The named step already encodes the right pace."]}),e.jsx(s,{children:"By intent"}),e.jsxs("div",{className:"border border-border rounded-xl px-5 mb-8",children:[e.jsx(d,{surface:"Hover / exit",token:"--motion-quick",note:"Lightweight feedback that should feel near-instant. (Alias of --st-duration-fast.)"}),e.jsx(d,{surface:"Default / enter",token:"--motion-base",note:"The default for most enter transitions and focus rings. (Alias of --st-duration-base.)"}),e.jsx(d,{surface:"Dialog / drawer",token:"--motion-expressive",note:"Larger surfaces sliding in; slightly slower reads as deliberate. (Alias of --st-duration-slow.)"}),e.jsx(d,{surface:"Looping",token:"--st-spinner-*",note:"Continuous loops (spinner ripple) use component-scoped timing, distinct from the enter/exit scale."})]}),e.jsx(s,{children:"Design principles"}),e.jsxs(t,{children:[e.jsx("strong",{children:"Respect prefers-reduced-motion."})," Every animated component disables motion under ",e.jsx("code",{children:"@media (prefers-reduced-motion: reduce)"})," — see ",e.jsx("code",{children:"Toast.css"}),","," ",e.jsx("code",{children:"Dialog.css"}),", ",e.jsx("code",{children:"Tooltip.css"}),". New motion must do the same; the demo on the ",e.jsx("strong",{children:"Duration Scale"})," page does it too."]}),e.jsxs(t,{children:[e.jsx("strong",{children:"Loop animations live in component CSS."})," Continuous loops (spinner) are authored as ",e.jsx("code",{children:"@keyframes"})," in the component's own ",e.jsx("code",{children:".css"}),", not as a transition token — see component-architecture §7."]}),e.jsxs(t,{children:[e.jsx("strong",{children:"Interactive feedback uses transition; enter/exit uses the scale."})," Reach for a ",e.jsx("code",{children:"transition"})," on hover/focus state; reach for ",e.jsx("code",{children:"--st-duration-*"})," on the mount/unmount keyframes."]}),e.jsxs(t,{children:[e.jsx("strong",{children:"Don't animate without a reason."})," Motion should communicate a state change. If a transition doesn't help the user understand what changed, leave it out."]}),e.jsx(s,{children:"Not yet tokenized"}),e.jsxs(t,{children:["Component ",e.jsx("em",{children:"hover"})," transitions are still ",e.jsx("strong",{children:"hardcoded"})," today (e.g. Badge at 150ms, others at 200ms) and do not yet reference ",e.jsx("code",{children:"--motion-*"}),". Folding them onto the motion scale is a separate spike. ",e.jsx("em",{children:"Enter/exit"})," animations (Tooltip / Dialog / Toast) already consume ",e.jsx("code",{children:"--st-duration-*"}),"."]}),e.jsx(s,{children:"Removed: --transition-*"}),e.jsxs(t,{children:["The older ",e.jsx("code",{children:"--transition-fast"})," / ",e.jsx("code",{children:"-normal"})," / ",e.jsx("code",{children:"-slow"})," and"," ",e.jsx("code",{children:"tokens.transition.*"})," / ",e.jsx("code",{children:"TransitionToken"})," were deprecated in v0.10.0 and ",e.jsx("strong",{children:"removed"})," in v0.11.0. Use ",e.jsx("code",{children:"--motion-*"})," (or"," ",e.jsx("code",{children:"tokens.motion.*"}),") instead. Note the old values bundled an easing (e.g."," ",e.jsx("code",{children:"150ms ease"}),") while the motion tokens are duration-only — specify your own easing: ",e.jsx("code",{children:"fast"})," → ",e.jsx("code",{children:"--motion-base"}),", ",e.jsx("code",{children:"normal"})," →"," ",e.jsx("code",{children:"--motion-expressive"}),", ",e.jsx("code",{children:"slow"})," (300ms) has no direct step."]})]})},T=["DurationScale","SemanticTokens","UsageGuide"];a.parameters={...a.parameters,docs:{...a.parameters?.docs,source:{originalSource:`{
  name: 'Duration Scale',
...
}`,...c.parameters?.docs?.source}}};export{a as DurationScale,i as SemanticTokens,c as UsageGuide,T as __namedExportsOrder,N as default};
