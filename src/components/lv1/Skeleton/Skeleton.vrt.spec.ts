...
const stories = ['text', 'shapes', 'card'] as const
...
for (const story of stories) {
  for (const theme of themes) {
    test(`Skeleton / ${story} / ${theme}`, async ({ page }) => {
      ...
      // Pause the shimmer for a deterministic still frame.
    ...
    test(`Skeleton / ${story} / ${theme} / a11y`, async ({ page }) => {
      await page.goto(storyUrl(story, theme))
      await page.waitForLoadState('networkidle')
