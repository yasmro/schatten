...
for (const story of stories) {
  for (const theme of themes) {
    test(`Switch parity / ${story} / ${theme}`, async ({ page }) => {
      ...
      // Freeze transitions so the thumb's translate-x and the check icon's
      // opacity land on the same still frame on both sides.
