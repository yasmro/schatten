...
const STORY_ID_PREFIX = 'components-lv1-table'
...
// Dark *rendering* is covered by Table.vrt.spec.ts. Parity runs light-only.
...
for (const story of stories) {
  for (const theme of themes) {
    test(`Table parity / ${story} / ${theme}`, async ({ page }) => {
