...
const STORY_ID_PREFIX = 'tokens-spacing'

// 'why-no-layout-primitives' — text only, no distinct visual
const stories = ['spacing-scale', 'recommended-scale', 'examples', 'half-steps'] as const
...
for (const story of stories) {
  for (const theme of themes) {
    test(`Spacing / ${story} / ${theme}`, async ({ page }) => {
